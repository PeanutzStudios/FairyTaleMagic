...
public class ShirtChange : MonoBehaviour
{
    ...
    // This will be the button that we will change the image on click
    public Button PreviewImageButton;  // Reference to the button
    ...
    void Start()
    {
        ...
        if (PreviewImageButton == null) Debug.LogError("PreviewImageButton is not assigned.");
        if (options.Count == 0) Debug.LogWarning("No sprites in options list. Please assign sprites in the Inspector.");
        
        // Set up the button listener to change the sprite when clicked
    ...
    private void SaveSprite()
    {
        ...
        {
            PlayerPrefs.SetInt("ShirtIndex", CurrentIndex);
            PlayerPrefs.Save();
    ...
    {
        if (PreviewImageButton != null && options.Count > 0 && CurrentIndex < options.Count)
        {
            // Update the button's image component to the selected sprite
            Image buttonImage = PreviewImageButton.GetComponent<Image>();
            if (buttonImage != null)
            {
                buttonImage.sprite = options[CurrentIndex]; // Switch the button's image
                Debug.Log("Updated preview image with sprite: " + options[CurrentIndex].name);
            }
            else
            {
                Debug.LogError("PreviewImageButton does not have an Image component.");
            }
        }
        else
        {
            Debug.LogWarning("PreviewImageButton is null or current index is out of range.");
        }
    }
