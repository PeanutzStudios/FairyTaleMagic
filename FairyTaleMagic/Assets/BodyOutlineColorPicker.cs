...
{
    public class ColorPickerForBodyOutline : MonoBehaviour
    {
        #region Variables
        ...
        private void Start()
        {
            ...
            // Find all objects with the tag "FrontHair"
            colorPickerObjects = GameObject.FindGameObjectsWithTag("BodyOutline");

            // Load the saved color when the game starts
            bodyColorOutlineStorage.Instance.LoadColor(); // Ensure color is loaded
            Color savedColor = bodyColorOutlineStorage.Instance.SelectedColor;
        ...
        public void Open()
        {
            Open(bodyColorOutlineStorage.Instance.SelectedColor); // Open with the saved color
        }
        ...
        {
            _currentColor = color;
            _colorResult.color = _currentColor; // Update UI Image color

            // Save the color using ColorStorage
            bodyColorOutlineStorage.Instance.SaveColor(color);

            // Update the SpriteRenderer and Image color on all objects with the tag "FrontHair"
        ...
        // New method to open ColorPicker using UI Button
        public void UI_Button_Open()
        {
            Open();
        }
